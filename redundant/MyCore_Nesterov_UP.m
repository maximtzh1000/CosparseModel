...
function [xk,niter,residuals,outputData,E_out,opts] = MyCore_Nesterov_UP(A,At,b,Lambda,La,mu,opts)
...
for k = 0:maxiter-1,
   ...
   %---- Updating yk 
    ...
    xk=xkp;
    if isa(U,'function_handle'),
        fx= Lambda*norm(U(xk),1) + 1/2*norm(res)^2;
    else
        fx= Lambda*norm(U*xk,1) + 1/2*norm(res)^2;
    end
    ...
    xold=xk;
    e=norm(xk-opts.x0)/norm(opts.x0);
    E_out=[E_out,e];    
